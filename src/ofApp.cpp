...
ofApp::ofApp(Graph* _G, Solution* _P, SafeZonesManager* _safe_zones_manager,
             bool _flg_capture_only)
    : G(_G),
      P(_P),
      safe_zones_manager(_safe_zones_manager),
      N(P->front().size()),
      T(P->size() - 1),
      goals(P->back()),
      scale(get_scale(G)),
...
void ofApp::setup()
{
    ...
    ofSetFrameRate(30);
    font.load("MuseoModerno-VariableFont_wght.ttf", font_size, true, false,
              true);
...
void ofApp::draw()
{
    ...
    for (int x = 0; x < G->width; ++x) {
        for (int y = 0; y < G->height; ++y) {
            ...
            bool regular_background = true;
            if (nullptr != safe_zones_manager){
                // draw safe zones
                auto safe_zone_agent_group_id =
                    safe_zones_manager->get_safe_agent_group(index,
                        timestep_slider);
                if (safe_zone_agent_group_id != -1) {
                    regular_background = false;
                    ofSetColor(Color::get_agent_group_color(safe_zone_agent_group_id));
                }
            }
            if (regular_background) {
                ofSetColor(Color::node);
            }
            auto x_draw =
                x * scale - scale / 2 + window_x_buffer + scale / 2 - 0.15;
            auto y_draw =
    ...
    if (flg_goal) {
        for (int i = 0; i < N; ++i) {
            ...
            ofDrawRectangle(x - goal_rad / 2, y - goal_rad / 2, goal_rad,
                            goal_rad);
            ...
            if (o != Orientation::NONE) {
                ...
                ofDrawTriangle(0, goal_rad / 2, 0, -goal_rad / 2, goal_rad / 2,
                               0);
                ofPopMatrix();
            }
        }
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (line_mode == LINE_MODE::STRAIGHT) {
            ofDrawLine(goals[i].v->x * scale + window_x_buffer + scale / 2,
                       goals[i].v->y * scale + window_y_top_buffer + scale / 2,
                       x, y);
        ...
        if (flg_font) {
            ofSetColor(Color::font);
            font.drawString(std::to_string(i), x - font_size / 2,
                            y + font_size / 2);
        }
    }
